...
for qq=1:length(PROFILE_NO) ;%ron
    rr=find(lo_float_mapped_data.la_profile_no==PROFILE_NO(qq)); %ron
    if size(rr)~=[1 1], error('cannot running_calibrate because cannot find matching mapped profile'), end; %ron
...
% geninv=v(:,1:2)*inv(s(1:2,1:2))*u(:,1:2)'*W;
   ...
   if(length(jj)>1) % only proceed if there are more than one data point
    ...
    errore = sqrt(diag(geninv*R2*geninv'));

    if(r>1)
      if((isempty(errore)==1)|(rank(W*G)<2))
        condslope_err0(i) = NaN;
        time_deriv_condslope_err(i) = NaN;
      else
        condslope_err0(i) = errore(1);
        time_deriv_condslope_err(i) = errore(2);
      end
    else
      condslope_err0(i) = errore; % for fitting a single profile
      time_deriv_condslope_err(i) = NaN;
    end
