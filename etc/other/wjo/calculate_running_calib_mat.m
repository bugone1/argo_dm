...
%                 C:\z\argo_dm\data\float_calib\freeland\calseries_*.mat
%Saves MAT file : C:\z\argo_dm\data\float_calib\freeland\cal_*.mat
function calculate_running_calib_mat( pn_float_dir, pn_float_name, po_system_configuration )
...
%MO changed reserved variable "error" for "erreur", july 2008
lo_float_source_data_filename= strcat( po_system_configuration.FLOAT_SOURCE_DIRECTORY, pn_float_dir, pn_float_name, po_system_configuration.FLOAT_SOURCE_POSTFIX );
lo_float_calseries_filename=strcat( po_system_configuration.FLOAT_CALIB_DIRECTORY, pn_float_dir, po_system_configuration.FLOAT_CALSERIES_PREFIX, pn_float_name, po_system_configuration.FLOAT_CALIB_POSTFIX);
...
lo_float_source_data=load(lo_float_source_data_filename) ;
lo_float_calseries=load(lo_float_calseries_filename) ;

if comparefiles(lo_float_source_data_filename,ls_float_calib_filename)
    ...
    if ~isequal(lo_float_calseries.calib_profile_no,lo_float_mapped_data.la_profile_no);error('cannot running_calibrate because profile numbers do not match');end; %ron
    if length(unique(PROFILE_NO))~=length(PROFILE_NO)
        error('Number of profile_no not unique');
    end
    if length(unique(lo_float_mapped_data.la_profile_no))~=length(lo_float_mapped_data.la_profile_no)
        warning('profile_no not continuous');
        end
    
    [tr,rr]=intersect(lo_float_mapped_data.la_profile_no,PROFILE_NO); %<MO> removed useless loop
    [qq,i]=sort(rr);
    INTERP_PRES(:,qq) =lo_float_mapped_data.la_INTERP_PRES(:,rr)  ; %ron
    INTERP_SAL(:,qq)  =lo_float_mapped_data.la_INTERP_SAL(:,rr)   ; %ron
    mapped_sal(:,qq)  =lo_float_mapped_data.la_mapped_sal(:,rr)   ; %ron
    mapsalerrors(:,qq)=lo_float_mapped_data.la_mapsalerrors(:,rr) ; %ron
    long_scale_large(:,qq)=lo_float_mapped_data.long_scale_large(:,rr); %ron
    lat_scale_large(:,qq) =lo_float_mapped_data.lat_scale_large(:,rr) ; %ron
    long_scale_small(:,qq)=lo_float_mapped_data.long_scale_small(:,rr); %ron
    lat_scale_small(:,qq) =lo_float_mapped_data.lat_scale_small(:,rr) ; %ron
    running_const(:,qq)=lo_float_calseries.running_const(:,rr);
    cal_series_flags(:,qq)=lo_float_calseries.cal_series_flags(:,rr);
    clear qq rr
    ...
    for i=1:n % use each profile as the centre
        ...
        if any(~isnan(SAL(:,i)))     % only proceed if the profile has data <mo> changed syntax
            ...
            r=b-a+1; % length of a:b should equal 2xrunning_const+1 when there are enough profiles
            x=cal_LONG(a:b);
            y=cal_LAT(a:b);
            ...
            jj=find(isfinite(cal_diffcond)); % weed out all the NaNs
            ...
            if length(jj)>1 % only proceed if there are more than one data point
                ...
                if r>1
                    if isempty(naturalsoln)||(rank(W*G)<2)
                        condslope(i)=NaN;
                        time_deriv_condslope(i)=NaN;
                    else
                ...
                erreur=sqrt(diag(geninv*R2*geninv'));
                if (r>1)
                    if isempty(erreur) || (rank(W*G)<2)
                        condslope_err0(i)=NaN;
                        time_deriv_condslope_err(i)=NaN;
                    else
                        condslope_err0(i)=erreur(1);
                        time_deriv_condslope_err(i)=erreur(2);
                    end
                else
                    condslope_err0(i)=erreur; % for fitting a single profile
                    time_deriv_condslope_err(i)=NaN;
                end
                if (r>1) && (r<2*running_const(i)+1)  % if the series is shorter than 2xrunning_const+1
                    midpt=(a+b)/2;        % find mid point of the series
                    condslope_err(i)=sqrt( condslope_err0(i).^2 + ((midpt-i)*time_deriv_condslope_err(i)).^2 );
                else
    ...
    cal_COND=(ones(k,1)*condslope).*COND; %profiles that were shallower than 400 dbar and do not get a cond_slope solution. It must be specified by the PI in ViewPlotsNew
   ...
   % save calibration data --------------------------------
    save( ls_float_calib_filename, 'cal_SAL', 'cal_SAL_err', 'running_const', 'condslope', 'condslope_err', 'time_deriv_condslope', 'time_deriv_condslope_err', 'cal_COND', 'cal_COND_err', 'PROFILE_NO' ) ;
end