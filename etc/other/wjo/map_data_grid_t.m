...
function [vgrid,vgriderror,vdata,vdataerror]=map_data_grid_t(v,posgrid,...
    posdata,regions,rank,gs,ts,q,s,e)

% **********************************************************************
% Objective mapping routine - need findindices.m and covarxyt.m.
...
% quite get implemented. (A.Wong, 11 April 2000)
% **********************************************************************
%
% Inputs:
%
...
%	disp('Creating data covariance matrices')
	...
	[Cdd] = s*covarxyt(posdata(dataindex,:),posdata(dataindex,:),gs,ts,q);
...
%		disp(['Mapping onto the data grid'])
		...
		for j=1:mm,
			[Cmd] = s*covarxyt(posdata(dataindex(j),:),posdata(dataindex,:),gs,ts,q);
			[vdata(dataindex(j),i)] = Cmd*wght+ meanv;
			[vdataerror(dataindex(j),i)]=sqrt(s-Cmd*Cdd*Cmd');
		end %for
%		disp(['Mapping onto the model grid'])
		for j=1:nn,
			[Cmd] = s*covarxyt(posgrid(gridindex(j),:),posdata(dataindex,:),gs,ts,q);
