...
% 16 Feb. 2018: Slight modification to the definition of an unflagged
%   inversion
% 19 Apr. 2018: Fixed up the reporting
...
for ii_prof=1:length(t)
    ...
    ii_inv_all = find(dens_ct_diff<=-0.03);
    for ii_inv=ii_inv_all
        if report_only==1 && any(t(ii_prof).pres_qc(ii_inv)=='1' & t(ii_prof).temp_qc(ii_inv)=='1' & t(ii_prof).psal_qc(ii_inv)=='1' ...
                & t(ii_prof).pres_qc(ii_inv+1)=='1' & t(ii_prof).temp_qc(ii_inv+1)=='1' & t(ii_prof).psal_qc(ii_inv+1)=='1')
