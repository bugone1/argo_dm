function nname=dfileConvDMQCModip(fname)
...
[~,ncalib1]=netcdf.inqDim(ncid1,netcdf.inqDimID(ncid1,'N_CALIB')); %length of n_calib in old file
...
for i=0:ndims1-1 %loop on old file dimensions
    ...
    if any(startsWith(dimname1{i+1},alldims)) %if this a dimension we're interested in
        j=j+1;
...
for varid1=0:nvars1-1 %loop on old file variables
    ...
    if any(startsWith(varname1,allvars)) %if this is a variable we're interested in
        [~,~,dimids2]=intersect(dimname1(dimids1+1),dimname2,'stable'); %find dimension indices in new file; must respect same dimension order
...
[~,~,dimids2]=intersect({'N_PROF','N_PARAM'},dimname2,'stable');
...
for varid1=0:nvars1-1   %loop on old file variables
    [varname1,~,dimids1] = netcdf.inqVar(ncid1,varid1);
    if any(startsWith(varname1,allvars)) %if this is a variable we're inerested in
...
[~,nparam2]=netcdf.inqDim(ncid2,netcdf.inqDimID(ncid2,'N_PARAM')); %length of n_calib
if size(coeffstrings,1)~=nparam2
    error('dimensions');
end
[~,nprof2]=netcdf.inqDim(ncid1,netcdf.inqDimID(ncid2,'N_PROF')); %length of n_calib
if nprof2~=1
