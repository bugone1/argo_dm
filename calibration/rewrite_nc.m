function fname=rewrite_nc(flnm,tem,qc,err,CalDate,conf,scical,PRES,adj_bfile,qc_flags_only)
...
%       qc_flags_only - Set to 1 to update QC flags without updating any of
...
% TODO: We're silently ignoring most of the input variables below if
% qc_flags_only is 1--is this OK?
if qc_flags_only && adj_bfile
...
if flnm.output(ok(end)+1)=='B'
    ...
    elseif qc_flags_only==1
        flnm.output(ok(end)+2)=flnm.input(ok(end)+2);
    end
else
    is_bfile=0;
    if qc_flags_only==1
...
%[trash,N_CALIB]=netcdf.inqDim(f,netcdf.inqDimID(f,'N_CALIB'));   % N_CALIB never seems to be used
...
if ~qc_flags_only, netcdf.putVar(f,parameterid,ndj,ndi,parms); end
...
for i=1:N_PARAM
    ...
    if ~strcmp(parm,'OTMP') && ~strcmp(parm,'BPHA') && ~(is_bfile && strcmp(parm,'PRES'))
        ...
        di(i_parlen)=256;
        if ~qc_flags_only && (~is_bfile || adj_bfile)
...
ok=ok(~isnan(ok));
if ~qc_flags_only && length(unique(ok))>1 && max(diff(unique(ok)))>.0011
    unique(ok)
    dbstop if error
