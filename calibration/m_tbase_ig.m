function [values,longs,lats]=m_tbase_ig(varargin);
...
% 01/Jun/17 - Added conversion of nlng from single to double to avoid an
%   error with fread - Isabelle Gaboury
...
%PATHNAME='C:\MATLAB\R2007b\toolbox\m_map\'; %MATHIEU OUELLET JULY 2008
%MATHIEU OUELLET AUGUST 2010:
w5=which('m_tbase');
ok=find(w5==filesep);
PATHNAME=w5(1:ok(end)); 
%/MO
...
if (eaxes(2)>4319 ),   % Read it in in 2 pieces!
  ...
  nlat=round((eaxes(3)-eaxes(4)))+1;
  nlng=double(round((eaxes(2)-eaxes(1)))+1);  % This sometimes turns out to be single-precision, which causes issues with fread below.
  values=zeros(nlat,nlng);
  for ii=[1:nlat],
   fseek(efid,(ii-1+eaxes(4))*(360*12*2)+eaxes(1)*2,'bof');
