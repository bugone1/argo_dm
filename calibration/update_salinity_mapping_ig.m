...
function update_salinity_mapping_ig( pn_float_dir, pn_float_name, po_system_configuration )
...
ln_float_level_count= size( lo_float_source_data.SAL,1);
...
for i = 1 : length( PROFILE_NO )
    if ~any(la_profile_no==PROFILE_NO(i))
...
for i = 1 : length( missing_profile_index )
    ...
    if ~isnan(LONG) && ~isnan(LAT) && any(~isnan(PRES)) && any(~isnan(SAL)) && any(~isnan(PTMP)) %MO added 20160919 to check if sal ro temp not empty
        ...
        if la_grid_lat~=999 % if no historical data is assigned to the float profile
            ...
            [elev,x,y] = m_tbase_ig( [min(la_grid_long1)-1, max(la_grid_long1)+1, min(la_grid_lat)-1, max(la_grid_lat)+1] );
            ...
            if any(la_bhist_long>360) && LONG>=0 && LONG<=20
                LONG=LONG+360;
            end
            ...
            for ln_level = 1:ln_float_level_count
                if ~isnan(SAL(ln_level)) && PRES(ln_level)>=map_p_exclude
                    ...
                    if length(la_hist_sal)>1 % only proceed with mapping if there are more than one data point
                        ...
                        for k = 1 : length(la_hist_long)
                            ...
                            if ~any( abs(c(:,1))<1/60 & abs(c(:,2))<1/60)
