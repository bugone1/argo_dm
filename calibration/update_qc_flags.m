function update_qc_flags(local_config,float_num, skip_tnpd_adj)
...
if nargin<3, skip_tnpd_adj=0; end

% Setup
rawflagpres_dir = local_config.RAWFLAGSPRES_DIR;
nc_dir_raw =local_config.DATA;
nc_dir_out = strcat(local_config.OUT,'changed\');
addpath('C:\Users\maz\Desktop\MEDS Project\vms_tools');
...
for ii_prof=1:length(t)
    ...
    ingested_flnm=dir([fullfile(nc_dir_raw,float_num) filesep '*' flnmp]); 
    clear flnm flnm_b
    if length(ingested_flnm)==2
        if strfind(ingested_flnm(1).name,'B')==1
            ...
            flnm.input = fullfile(nc_dir_raw,float_num,ingested_flnm(2).name);
            flnm.output = fullfile(nc_dir_out,ingested_flnm(2).name);
            %flnm_b.input=[nc_dir_raw findnameofsubdir(float_num,listdirs(nc_dir_raw)) filesep ingested_flnm(1).name];
            flnm_b.input = fullfile(nc_dir_raw,float_num,ingested_flnm(1).name);
            flnm_b.output = fullfile(nc_dir_out, ingested_flnm(1).name);
        ...
        flnm.input = fullfile(nc_dir_raw,float_num,ingested_flnm(1).name);
        flnm.output = fullfile(nc_dir_out, ingested_flnm(1).name);
        flnm_b.input = [];
        flnm_b.output = [];
    end
...
%         end
        ...
        % If we've changed a flag from '4', we need to reset
        % the adjusted values
        tem={};
        for temp_var = {'pres','temp','psal'}
            tem.(upper(temp_var{1})) = t(ii_prof).([temp_var{1},'_adjusted']);
            ii_improved = t(ii_prof).([temp_var{1},'_qc'])<'4' & t_old.([temp_var{1},'_qc'])=='4';
            if any(ii_improved)
                if all(ii_improved)
