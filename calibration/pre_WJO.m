...
while ii < length(netcdf_names)
    ...
    [LATnew,LONGnew,DATESnew,PRESnew,SALnew,TEMPnew,PTMPnew,PROFILE_NOnew,PI_NAMEnew,WMO_IDnew,DC_REFERENCEnew] = ncprofile_read_OSAP([local_config.NEW netcdf_names{ii}]);
    ...
    if (float_count==0 || ~strcmp(next_name,float_names{float_count})) %float names were sorted above so that each profile is grouped with the other profiles from a given float
        ...
        if float_count>Num_Floats_To_Run;break;end
        float_dirs{float_count}=local_config.PI;
        float_names{float_count}=next_name;
    end
    %if (strmatch('intercomparison',local_config.RUN_MODE)); PI_NAMEnew= 'freeland';end % to accept any PI name
    if ~(isempty(PTMPnew) || length(PRESnew)>N100 || PROFILE_NOnew>=9000) % & [strfind(lower(PI_NAMEnew),lower(PI_name)) strfind(lower(PI_NAMEnew),'radhakrishnan') strfind(lower(PI_NAMEnew),'rojas')]  %catch all the unusable files. Ricardo Rojas runs the Chilean program.
        ...
        if ~strcmp(flnm,old_flnm) %we have a new float
            ...
            if exist(flnm)
                ...
                SAL(isnan(TEMP))=NaN; %make sure no bad temperatures have been used.
                SAL(isnan(PRES))=NaN; %make sure no bad pressures have been used.
        ...
        system(['copy ' local_config.NEW netcdf_names{ii} ' ' local_config.INGESTED]);
        ...
        system(['move ' local_config.NEW netcdf_names{ii} ' ' local_config.DNP]);
        netcdf_names=netcdf_names([1:ii-1 ii+1:end]);
        %netcdf_names{ii}='unusable'; %remove this file from further processing by giving it a special name without a '_'
...
printout=temporary(:)';
display(['Ready to start argo_calibration on the following new files: ' printout]);