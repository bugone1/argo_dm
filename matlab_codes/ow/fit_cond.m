    ...
    elseif nbr == 0
        ...
        if setbreaks
            if nbr1 == max_brk;  % break points are set get linear-lsq fit to offset and slopes
                ...
                catch mess % if error in lsqnonlin get last iteration
                    %#### need to fix the following
                    if strcmp(mess.identifier,'MATLAB:license:checkouterror')
                    error('license checked out')
                    end
            ...
            try
                ...
                if strcmp(mess.identifier,'MATLAB:license:checkouterror')
                    error('license checked out')
                else strcmp(mess.identifier,'MATLAB:UndefinedFunction')
                    error('license checked out?')
                end
                [ubrk resnorm residual] = LMA(ubrk_g);
            end
    ...
    for i = 1:nloops
        ...
        elseif setbreaks
            ...
            try
                ...
                if strcmp(mess.identifier,'MATLAB:license:checkouterror') || ...
                        strcmp(mess.identifier,'MATLAB:UndefinedFunction')                    
                    error('license checked out')
                end
                'A before'
                A
