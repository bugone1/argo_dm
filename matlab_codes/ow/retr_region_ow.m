...
pa_index_0 = 0;
[pa_grid_sal,pa_grid_ptmp,pa_grid_pres,pa_grid_lat,pa_grid_long,pa_grid_dates]=deal([ ]);
        ...
        if( ~isnan(pa_wmo_numbers(ln_index,1)) && pa_wmo_numbers(ln_index,ntyp) ) % check to see if we are supposed to load this data type
            ...
            if ntyp == 2 % the 2nd column denotes CTD data
                try
                    ...
                    display(['missing box ' snumb '; hit enter']);
                    allok=false;
                end
            ...
            if allok
                ...
                for n=1:i1  % load each station
                    ok=index==pa_index(n);
                    if any(ok) % this entry is in the index list
                        %aw                if n > length(lo_box_data.lat); keyboard; end
                        jj = ~isnan(lo_box_data.pres(:,n)); % only retrieve non-NaN values
...
pa_grid_ptmp( ln_ii ) = NaN;
ln_ii = isnan( pa_grid_pres );
pa_grid_sal( ln_ii ) = NaN;
pa_grid_ptmp( ln_ii ) = NaN;
ln_ii = isnan( pa_grid_ptmp ) ;
pa_grid_sal( ln_ii ) = NaN;
pa_grid_pres( ln_ii ) = NaN;
