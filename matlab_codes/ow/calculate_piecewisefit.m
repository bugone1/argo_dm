...
if exist(ls_float_calib_filename,'file')
    dd=dir(ls_float_calib_filename);
    de=dir(strcat( po_system_configuration.FLOAT_SOURCE_DIRECTORY, pn_float_dir, pn_float_name, po_system_configuration.FLOAT_SOURCE_POSTFIX) );
    if dd.datenum>de.datenum
%        return
    end
end
...
PROFILE_NO = double(lo_float_source_data.PROFILE_NO); % profile number
...
if ~sstatus % set_calseries returned a bad status variable, go ahead and write out file with NaNs
    ls_float_calib_filename = strcat( po_system_configuration.FLOAT_CALIB_DIRECTORY, pn_float_dir, po_system_configuration.FLOAT_CALIB_PREFIX, pn_float_name, po_system_configuration.FLOAT_CALIB_POSTFIX ) ;
